...
public class TimerTask implements Runnable {
    ...
    @Override
    public void run() {
        if (game == null || (game.getStatus() != Status.FINAL_COUNTDOWN && game.getStatus() != Status.RUNNING && game.getStatus() != Status.FINAL))
        ...
        if (this.remainingtime == 0) {
            stop();
            game.stop(false);
        } else if (this.remainingtime % 30 == 0) {
            ...
            if (minutes != 0) {
                ...
                game.tipAll(HG.getInstance().getLanguage().getGame_ending_sec().replace("%seconds%", String.valueOf(this.remainingtime)));
